...
type FontSizeWithPrefix = `${'text-'}${keyof DefaultTheme['fontSize']}` | undefined

type FontWeightWithPrefix = `${'font-'}${keyof DefaultTheme['fontWeight']}` | undefined
...
export const Txt: FC<TProps> = ({ elm, children, size, weight, className }) => {
  ...
  return <Element className={`${className ?? ''} ${size} ${weight}`}>{children}</Element>
}
